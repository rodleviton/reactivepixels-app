...
app.directive('uploadForm', function($q, $timeout, $firebase, $upload, FIREBASE_URL, SignedRequestService, UserService) {
    return {
        restrict: 'E',
        replace: true,
        ...
        link: function(scope, element, attrs) {
            ...
            // Signed Request Url
            var signedRequestsRef = new Firebase("https://reactivepixels.firebaseio.com/users/" + scope.authUser.username + '/signed-requests/' );

            // Upload progeress indicator
            ...
            /*
             * Get Signed Request once file is selected
             */
            scope.imageUploads = [];
            var file;
            ...
            scope.onFileSelect = function($file) {
                if($file[0]) {
                    ...
                    getSignedRequest(file[0].type) // Grab an AWS signed request

                    // Wait for signed request to be returned to user
                    signedRequestsRef.on('child_added', function(childSnapshot, prevChildName) {

                        // Only grab the signed request we are after
                        if (childSnapshot.val().id == scope.token) {

                            // Upload the file
                            uploadFile( childSnapshot.val() );
                 ...
                 */
                 ...
                 var uploadFile = function(s3Params) {
                    ...
                    scope.upload = $upload.upload({
                        ...
                        method: 'POST',
                        data: {
                            'key': 'tmp/' + scope.token + '-' + file[0].name,
                    ...
                    scope.upload.then(function(response) {
                        ...
                        if (response.status === 201) {
                            ...
                            // Display the thumbnail for cropping
                            displayImage(scope.parsedData);

                        } else {
                            console.log('Upload Failed');
                        }

                        // Remove the used signed request from users queue
                        signedRequestsRef.child(scope.token).remove();
            ...
            $timeout(function() {
                galleryCropper = new CROP();
                galleryCropper.init('.gallery-cropper');


                // Cropping function
                $('#crop').on('click', function() {
                    var coords = coordinates(galleryCropper);
                    // Add to queue for processing
                    imageProcessRef.child(scope.token).set(
                        {
                            id: scope.token,
                            username: scope.authUser.username,
                            filename: scope.token + '-' + file[0].name,
                            url: scope.parsedData.location,
                            x: coords.x,
                            y: coords.y,
                            w: coords.w,
                            h: coords.h
            ...
            // // Wait for process image to be returned to user process queue before writing to db
                            // imageProcessRef.on('child_added', function(childSnapshot, prevChildName) {
                            //     if (childSnapshot.val().id == scope.token) {
            ...
            var displayImage = function(image) {
                ...
                // Display Thumbnail
                galleryCropper.loadImg(image.location);
            };
             ...
             */
            var getSignedRequest = function(fileType) {

                // Get a signed request
                SignedRequestService.get(fileType, scope.authUser.username, scope.token).then(function(error) {
                    if(error) {
            ...
            var generateToken = function() {
                var _token = "";
                var _possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";

                for (var i = 0; i < 30; i++) {
                    _token += _possible.charAt(Math.floor(Math.random() * _possible.length));
                }

                return _token;
            }

            // Token to be used through upload process
            scope.token = generateToken();
